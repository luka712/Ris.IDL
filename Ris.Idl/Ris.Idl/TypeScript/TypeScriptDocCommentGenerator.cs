...
{
    /// <summary>
    /// Generates a JSDoc comment from a <see cref="IdlDocCommentSymbol"/>.
    /// </summary>
    /// <param name="docCommentSymbol">The <see cref="IdlDocCommentSymbol"/>.</param>
    /// <returns>The generated code.</returns>
    public string GenerateDocComment(IdlDocCommentSymbol docCommentSymbol)
    {
        var sb = new StringBuilder();
        sb.AppendLine("/**");
        sb.AppendLine($" * {docCommentSymbol.Summary}");

        if (docCommentSymbol.Remarks is not null)
        {
            foreach (var remarks in docCommentSymbol.Remarks)
            {
                sb.AppendLine($" * {remarks}");
            }
        }

        if (docCommentSymbol.Parameters is not null)
        {
            foreach (var parameter in docCommentSymbol.Parameters)
            {
                sb.AppendLine($" * @param {parameter.Name} - {parameter.Description}");
            }
        }
        
        if (docCommentSymbol.Returns != null)
        {
            sb.AppendLine($" * @returns {docCommentSymbol.Returns}");
        }
