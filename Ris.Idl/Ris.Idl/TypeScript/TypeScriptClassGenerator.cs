...
public class TypeScriptClassGenerator : ITypeGenerator
{
    ...
    {
        var @class = type as IdlClassSymbol;

        if (@class is null)
        {
            throw new ArgumentException($"Expected {nameof(IdlClassSymbol)}, got {type.GetType().Name}", nameof(type));
        }
    ...
    {
        return type is IdlClassSymbol;
    }

    private void GenerateProperties(IdlClassSymbol type, StringBuilder sb, string indent, TypeScriptConfig tsConfig)
    {
        ...
        for (var i = 0; i < properties.Count; i++)
        {
            ...
            sb.AppendLine($"{indent}{@static} get {propertyName}{nullable}(): {propertyType} {{");
            sb.AppendLine($"{indent}    return this._{propertyName};");
            sb.AppendLine($"}}");
            ...
            if (hasSetter)
            {
                ...
                sb.AppendLine($"{indent}{@static} set {propertyName}{nullable}(value: {propertyType}): void {{");
                sb.AppendLine($"{indent}  this._{propertyName} = value;");
                sb.AppendLine($"}}");
            }
    ...
    private void GenerateEvents(IdlClassSymbol type, StringBuilder sb, string indent, TypeScriptConfig tsConfig)
    {
        // Generate events
    ...
    private void GenerateMethods(IdlClassSymbol type, StringBuilder sb, string indent, TypeScriptConfig tsConfig)
    {
